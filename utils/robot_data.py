...
class RobotData():
    ...
    def __init__(
        ...
        self.__contact_history = np.zeros((2, 3), dtype=np.float32)
    ...
    def update(
        ...
        # generalized joint position (floating base), dim: 3 + 4 + 6 = 13.
    ...
    def __init_pinocchio(self, robot_model) -> None:
        ...
        # The function is to build the model from the urdf file.
        self.__pin_model = pinocchio.buildModelFromUrdf(
            robot_model, pinocchio.JointModelFreeFlyer()) # Make the model
        self.__pin_data = self.__pin_model.createData() # Make the data including joints, frames, etc.
        
        
    def __compute_foot_Jacobian(self) -> Tuple[List[np.ndarray], List[np.ndarray]]:
        foot_frames = ['foot_L_Joint', 'foot_R_Joint']
        foot_frames_ID = [self.__pin_model.getFrameId(foot_frames[i]) for i in range(2)]
    ...
    def __compute_base_vel_base_feet(self) -> List[np.ndarray]:
        base_vel_base_feet = []
        for foot_idx in range(2):
            generalized_qdot = list(self.lin_vel_base) + list(self.ang_vel_base) + list(self.qdot)  # 3+3+6=12
            generalized_qdot = np.array(generalized_qdot, dtype=np.float32)
            print("generalized_qdot: ", generalized_qdot.shape)
            print("selflin_vel_base: ", self.lin_vel_base.shape)
            print("self.ang_vel_base: ", self.ang_vel_base.shape)
            print("self.qdot: ", self.qdot.shape)
            print("Jv_feet[foot_idx]: ", self.Jv_feet[foot_idx].shape)
            print("self._contact_history: ", self.__contact_history.shape)
    ...
    def __compute_pos_thighs(self) -> List[np.ndarray]:
        thigh_frames = ['hip_L_Joint', 'hip_R_Joint']
        thigh_frames_ID = [self.__pin_model.getFrameId(thigh_frames[i]) for i in range(2)]
        pos_thighs = []
...
# def test():
#     robot_path = os.path.join(os.path.dirname(__file__), '../PF_P441C2/urdf/robot.urdf')
