...
class Gait(Enum):
    ...
    STANDING = 'standing', 16, np.array([0, 0]), np.array([16, 16])
    TROTTING16 = 'trotting', 16, np.array([0, 8]), np.array([8, 8])
    TROTTING10 = 'trotting', 10, np.array([0, 5]), np.array([5, 5])
    JUMPING16 = 'jumping', 16, np.array([0, 0]), np.array([4, 4])
    # BOUNDING8 = 'bounding', 8, np.array([4, 4]), np.array([4, 4])
    PACING16 = 'pacing', 16, np.array([8, 0]), np.array([8, 8])
    PACING10 = 'pacing', 10, np.array([5, 0]), np.array([5, 5])

    def __init__(
        self, 
        name: str, 
        num_segment: int,    # the number of execution segments of MPC
        stance_offsets: np.ndarray,    
        stance_durations: np.ndarray   # stance time for each leg
    ) -> None:

        self.__name = name
        self.__num_segment = num_segment
        self.__stance_offsets = stance_offsets   # stance offset for each leg in one period
        self.__stance_durations = stance_durations

        self.__load_parameters()

        # each leg has the same swing time and stance time in one period
        # every period has different segments
    ...
    def get_gait_table(self) -> np.ndarray:
        ...
        '''
        gait_table = np.zeros(2 * self.__mpc_horizon, dtype=np.float32)
    ...
    def get_swing_state(self) -> np.ndarray:
        ...
        phase_state = np.array([self.phase, self.phase], dtype=np.float32)
