...
class ModelPredictiveController():
    ...
    def __init__(self, mpc_config: LinearMpcConfig, robot_config: PF_P441CConfig):        
        # state: x(t) = [\theta, p, \omega, \dot{p}, g], dim: 13
        self.num_state = 3 + 3 + 3 + 3 + 1
        # input: u(t) = [f1, f2], dim: 6
        self.num_input = 3 + 3  
    ...
    def _load_parameters(self, mpc_config: LinearMpcConfig, robot_config: PF_P441CConfig):        
        ...
        self.fz_max = robot_config.fz_max   # maximum normal force
        print("self.fz_max:",self.fz_max)
    ...
    def update_mpc_if_needed(self, iter_counter, base_vel_base_des, yaw_turn_rate_des, 
        ...
        print("vel_base_des:",vel_base_des)
        ...
        if iter_counter % self.iterations_between_mpc == 0:
            ...
            print("solver:",solver)
            self.__contact_forces = self._solve_mpc(ref_traj, gait_table, solver=solver)[0:6]
            solve_end = time.time()
            print('MPC solved in {:3f}s.'.format(solve_end - solve_start))
            print("self.yaw:",self.yaw, "self.yaw_desired:",self.yaw_desired)
        ...
        return self.__contact_forces[0:6]
    ...
    def _discretize_continuous_model(self, Ac, Bc):
        # square_matrix = [[Ac (13*13), Bc (13*6)],
        #                  [0  (6*13), 0  (6*6)]] * dt (19*19)
        dim = self.num_state + self.num_input  # 19
    ...
    def _generate_QP_cost(self, Ad, Bd, xt, Xref, debug=False):
        # power_of_A = [I, A, A^2, A^3, ..., A^k]
        power_of_A = [np.identity(self.num_state, dtype=np.float32)] # 13 * 13
    ...
    def _generate_QP_constraints(self, gait_table):
        ...
        constraint_coef_matrix = np.array([
            [ 1,  0, self.mu],
            [ 0,  1, self.mu],
            [ -1, 0, self.mu],  # y方向的正向摩擦约束
            [ 0, -1, self.mu],  # y方向的负向摩擦约束
            [ 0,  0,       1]
        ], dtype=np.float32)
        qp_C = np.kron(np.identity(2 * self.horizon, dtype=np.float32), constraint_coef_matrix)
        
        C_lb = np.zeros(2 * 5 * self.horizon, dtype=np.float32)
        C_ub = np.zeros(2 * 5 * self.horizon, dtype=np.float32)
        k = 0
        for i in range(self.horizon):
            for j in range(2):    # number of legs
                C_ub[5*k] = np.inf
                C_ub[5*k+1] = np.inf
                C_ub[5*k+2] = np.inf            
                C_ub[5*k+3] = np.inf
                C_ub[5*k+4] = gait_table[2*i+j] * self.fz_max
                k += 1
    ...
    def _solve_mpc(self, ref_traj, gait_table, solver='drake', debug=False):
        ...
        qpH = 0.5 * (qpH + qpH.T)
        epsilon = 1e-4
        qpH += epsilon * np.eye(qpH.shape[0])


        
        eigenvalues = np.linalg.eigvals(qpH)
        print("Eigenvalues of qp_H:", eigenvalues)
        # end_time = time.time()
        ...
        if solver == 'drake':
            # 定义决策变量
            contact_forces = cp.Variable(self.num_input * self.horizon)

            # 定义目标函数
            objective = 0.5 * cp.quad_form(contact_forces, cp.psd_wrap(qpH)) + qpg @ contact_forces

            # 定义约束
            constraints = [
                qp_C @ contact_forces >= C_lb,
                qp_C @ contact_forces <= C_ub
            ]
